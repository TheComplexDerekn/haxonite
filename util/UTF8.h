...
// Parse one UTF-8 character from [s] (of length [length]), starting
// at [idx].
//
// If [idx] is at (or past) the end of [s]: returns false.
//
// If there is a valid UTF-8 character starting at [idx]: stores the
// character in [u], updates [idx] to point to the following byte, and
// returns true.
//
...
// Returns the length of the UTF-8 character starting at [idx] in [s].
// This follows the same rules as utf8Get: returns 1 for an invalid
// UTF-8 character. Returns 0 if [idx] is at (or past) the end of [s].
...
// not a valid Unicode character (i.e., if [i] is too large).
